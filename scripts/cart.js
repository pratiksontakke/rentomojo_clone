import addToCart from "./demoAddtocart.js";
...
function displayTable(addToCartFromLS) {
    ...
    addToCartFromLS.forEach(function (elem, index) {
        ...
        dustbin.src = "https://www.rentomojo.com/public/images/radical-cart/icons/delete__icon.svg"
...
subcontainer.append(container1)
    ...
    document.getElementById("total_amount").innerText = `${tt} \n Payable Now`;
...
function refundable(addToCartFromLS) {
    ...
    for (let i = 0; i < addToCartFromLS.length; i++){
        bag2 = bag2 + (addToCartFromLS[i]["quantity"] * addToCartFromLS[i]["deposit"])
    }
    return bag2;
...
function monthly(addToCartFromLS) {
    let bag4 = 0;
    for (let i = 0; i < addToCartFromLS.length; i++) {
        bag4 = bag4 + (addToCartFromLS[i]["quantity"] * addToCartFromLS[i]["price"])
    }
    return bag4;
...
function  Gst(addToCartFromLS) {
    let bag5 = 0;
    for (let i = 0; i < addToCartFromLS.length; i++){
        bag5 = bag5 + (addToCartFromLS[i]["quantity"] * addToCartFromLS[i]["price"])
...
function TotalAmount(addToCartFromLS) {
    let bag = 0;
    for (let i = 0; i < addToCartFromLS.length; i++){
        bag = bag + (addToCartFromLS[i]["quantity"] * addToCartFromLS[i]["price"])
    }
    return bag;
